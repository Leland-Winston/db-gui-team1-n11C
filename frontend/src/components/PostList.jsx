...
export const PostList = ({ posts, context }) => {
  ...
  let [filters, setFilters] = useState(init);
  ...
  //         setModels(getModelsFromGarage(context.garage));
  //       //.then((x) => {
  //       //   if (!!x[0]) {
  //       //     x.forEach((m) => {
  ...
  const getFilteredPosts = () => {
    ...
    if (filters.model && filters.year) {
      let x = getIdsFromModel(filters.model);
      x = JSON.parse(x);
      let __filteredPosts = _filteredPosts.filter((post) => x == post.car_id);
      setFilteredPosts(__filteredPosts);
    } else {
  ...
  useEffect(() => {
    ...
    //   modelIds.push(getIdsFromModel(model));
    // });
  }, []);
